...
echo -en "$HSTAR Retrieving the fingerprint (3/12): " #{{{
[[ -f $HASH ]] && rm $HASH
getFile $LOCATION/$HASH
#}}}

echo -en "$HSTAR Retrieving stage 4 (4/12): " #{{{
retrieveFile $FILE $HASH $LOCATION
#}}}

echo -en "$HSTAR Checking the new file's fingerprint (5/12): " #{{{
checkFingerprint $HASH
#}}}

echo -en "$HSTAR Retrieving the file's signature (6/12): " #{{{
getFile $LOCATION/$SIG
#}}}

echo -en "$HSTAR Checking file's signature (7/12): " #{{{
